...
/* *******************************************************************
    ...
    operations for sparse matrix and calculation of other matrix 
...
class SparseMatrix{
    ...
    public:
        ...
        double& operator()(int i, int j){
            ...
            int index = sparseAddr.index(i,j) ;
            if(index != -1){
                return A_[index];
            }
            return zero_ ;
        ...
        SparseMatrix operator*(const double s) const{
        ...
        bool strictDiagonalDominance() const{
            ...
            for(int i=0; i < n_; ++i){
                for(int j=sparseAddr.rowArray()[i]; j < sparseAddr.rowArray()[i+1]; ++j){
                    sum += abs((*this)(i, sparseAddr.columnIndexArray()[j])) ;
                }
                if(abs(d_[i]) <= sum){
                    return false ;
                }
        ...
        void readMatrix(istream& in){
            ...
            while(getline(in,line)){
                ...
                if(line.empty() || line[0] == '%'){
                    ...
                    if(strVec[0]!=(strVec[1]) || stoi(strVec[0])!=n_) std::__throw_runtime_error("inconsistent matrix dimensions") ;
